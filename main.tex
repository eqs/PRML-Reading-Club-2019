...
\begin{document}
    ...
    \begin{frame}{Motivation}
        \begin{itemize}
            \item Purpose: \textit{density estimation}
                \begin{itemize}
                    \item To model the probability distribution $p(\mathbf x)$ of a random var $\mathbf x$, 
                        given a finite set of observations.
                \end{itemize}
            \item Parametric and non-parametric models
            \item Frequentist and Bayesian treatments
                \begin{itemize}
                    \item frequentist: choose specific values of the params by optimizing some criterion (e.g. the likelihood)
                    \item Bayesian: introduce prior over the params and compute the corresponding posterior
                \end{itemize}
        \end{itemize}
    \end{frame}
    
    \begin{frame}{Image for Probabilistic Estimation}
        \begin{itemize}
            \item $\mathbf x$: location of a target (unknown)
            \item $\mathbf y$: value of a sensor (known)
                \begin{figure}[h]\centering
                    \includegraphics[width=9.0cm]{./figs/bayes.pdf}
                \end{figure}
            \item Goal: Estimate the location using observations
            \item All computations are analytically tractable for Gaussians.
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{}
        \begin{center}
            \includegraphics[height=4.5cm]{./figs/nara_genkai.png}
            {\scriptsize \url{https://twitter.com/nara_genkai/status/1059076997060583426}}
        \end{center}
    \end{frame}
    ...
    \begin{frame}{Derivation of Conditional Gaussian Distribution}
        ...
        \begin{align*}
            ...
            %---------------
            \onslide<2->{
            &= -\frac{1}{2}\left( \highlight[red]{\mathbf x_a^\top\bm\Lambda_{aa}\mathbf x_a} - \highlight[blue]{\mathbf x_a^\top\bm\Lambda_{aa}\bm\mu_a} - 
                               \highlight[blue]{\bm\mu_a^\top\bm\Lambda_{aa}\mathbf x_a} + \bm\mu_a^\top\bm\Lambda_{aa}\bm\mu_a \right) \\
            &\qquad -\frac{1}{2}\left( \mathbf x_a^\top\bm\Lambda_{ab}\mathbf x_b - \highlight[blue]{\mathbf x_a^\top\bm\Lambda_{ab}\bm\mu_b} - 
            ...
            &\qquad -\frac{1}{2}\left( \mathbf x_b^\top\bm\Lambda_{ba}\mathbf x_a - \mathbf x_b^\top\bm\Lambda_{ba}\bm\mu_a - 
                               \highlight[blue]{\bm\mu_b^\top\bm\Lambda_{ba}\mathbf x_a} + \bm\mu_b^\top\bm\Lambda_{ba}\bm\mu_a \right) \\
            ...
            &= \highlightcap[red]{\displaystyle -\frac{1}{2}\mathbf x_a^\top\bm\Lambda_{aa}\mathbf x_a}{\small The quadratic term}
                + \highlightcap[blue]{\mathbf x_a^\top(\bm\Lambda_{aa}\bm\mu_a + \bm\Lambda_{ab}\bm\mu_b)}{The linear term}
                + \mathrm{const.}
            }
        \end{align*}
    \end{frame}
    ...
    \begin{frame}{(Recall) Image for Probabilistic Estimation}
        \begin{itemize}
            \item $\mathbf x$: location of a target (unknown)
            \item $\mathbf y$: value of a sensor (known)
                \begin{figure}[h]\centering
    ...
    \begin{frame}{Demonstration of a Bayes' theorem for Gaussian variables}
        See Jupyter Notebook.

        {\scriptsize\url{https://gist.github.com/eqs/a9f66d8a702da6ed7ff5a5b92aaa5811}}
    \end{frame}
    ...
    \begin{frame}{A frequentist treatment vs.\ a Bayesian treatment}
        To model the dist of a random var using \textit{parametric} dist, given observations, 
        \begin{itemize}
            \item frequentist: choose specific values of the params by optimizing some criterion (e.g. the likelihood)
            \item Bayesian: introduce prior over the params and compute the corresponding posterior
        \end{itemize}
    ...
    \begin{supframe}{Solution of Exercise 2.35 (1/3)}
        \begin{itemize}
            \item Derivation of (2.62) using (2.59): See the text.
            \item Derivation of (2.291)
                \begin{align*}
                    \mathbb E[\mathbf x_n\mathbf x_m^\top] = \bm\mu\bm\mu^\top + I_{nm}\bm\Sigma \tag{2.291}
                \end{align*}
                \begin{enumerate}
                    \item[(i)] In the case $n = m$, from (2.62), we have
                        \begin{align*}
                            \mathbb E[\mathbf x_n\mathbf x_m^\top] = \bm\mu\bm\mu^\top + \bm\Sigma
                        \end{align*}
                    \item[(ii)] In the case $n \not = m$, 
                        \begin{align*}
                            &\mathbb E[\mathbf x_n\mathbf x_m^\top] \\
                            &= \iint p(\mathbf x_n, \mathbf x_m)\mathbf x_n\mathbf x_m^\top \:\mathrm d\mathbf x_n\mathrm d\mathbf x_m \\
                            &= \nbracket{\int p(\mathbf x_n)\mathbf x_n \:\mathrm d\mathbf x_n}
                                \nbracket{\int p(\mathbf x_m)\mathbf x_m^\top \:\mathrm d\mathbf x_m} \;\;(\because \mbox{i.i.d.}) \\
                            &= \mathbb E[\mathbf x_n]\mathbb E[\mathbf x_m]^\top \\
                            &= \bm\mu\bm\mu^\top
                        \end{align*}
                \end{enumerate}
            \item Derivation of (2.124): next slide
        \end{itemize}
    \end{supframe}
    
    \begin{supframe}{Solution of Exercise 2.35 (2/3)}
        \begin{align*}
            &\mathbb E[\bm\Sigma_\mathrm{ML}] \\
            &= \mathbb E\rbracket{\frac{1}{N}\sum_{n=1}^N (\mathbf x_n - \bm\mu_\mathrm{ML})(\mathbf x_n - \bm\mu_\mathrm{ML})^\top} \\
            &= \frac{1}{N}\sum_{n=1}^N \mathbb E\rbracket{(\mathbf x_n - \bm\mu_\mathrm{ML})(\mathbf x_n - \bm\mu_\mathrm{ML})^\top} \\
            &= \frac{1}{N}\sum_{n=1}^N 
                \mathbb E\rbracket{\cbracket{(\mathbf x_n - \bm\mu_\mathrm{ML}) - (\bm\mu_\mathrm{ML} - \bm\mu)}
                                    \cbracket{(\mathbf x_n - \bm\mu_\mathrm{ML}) - (\bm\mu_\mathrm{ML} - \bm\mu)}^\top} \\
            &= \highlightcap[red]{\displaystyle\frac{1}{N}\sum_{n=1}^N \mathbb E\rbracket{(\mathbf x_n - \bm\mu)(\mathbf x_n - \bm\mu)^\top}}{$\bm\Sigma$}
                - 2\cdot\highlightcap[red]{\displaystyle\frac{1}{N}\sum_{n=1}^N \mathbb E\rbracket{(\mathbf x_n - \bm\mu)(\bm\mu_\mathrm{ML} - \bm\mu)^\top}}{$\mathbb E\rbracket{(\bm\mu_\mathrm{ML} - \bm\mu)(\bm\mu_\mathrm{ML} - \bm\mu)^\top}$} \\
            & \quad + \frac{1}{N}\sum_{n=1}^N \mathbb E\rbracket{(\bm\mu_\mathrm{ML} - \bm\mu)(\bm\mu_\mathrm{ML} - \bm\mu)^\top}
        \end{align*}
    \end{supframe}
    
    \begin{supframe}{Solution of Exercise 2.35 (3/3)}
        \begin{align*}
            &\mathbb E[\bm\Sigma_\mathrm{ML}] \\
            &= \bm\Sigma - \mathbb E\rbracket{(\bm\mu_\mathrm{ML} - \bm\mu)(\bm\mu_\mathrm{ML} - \bm\mu)^\top} \\
            &= \bm\Sigma - \nbracket{\mathbb E\rbracket{\bm\mu_\mathrm{ML}\bm\mu_\mathrm{ML}^\top} - \bm\mu\bm\mu^\top} \\
            &= \bm\Sigma - \nbracket{\frac{1}{N^2}\sum_{n=1}^N\sum_{m=1}^N \mathbb E\rbracket{\mathbf x_n\mathbf x_m^\top} - \bm\mu\bm\mu^\top} \\
            &= \bm\Sigma - \nbracket{\frac{1}{N^2}\sum_{n=1}^N\sum_{m=1}^N \nbracket{\bm\mu\bm\mu^\top + I_{nm}\bm\Sigma} - \bm\mu\bm\mu^\top} \;\; (\because \mbox{2.291})\\
            &= \bm\Sigma - \nbracket{\frac{1}{N^2}\nbracket{N^2\bm\mu\bm\mu^\top + N\bm\Sigma} - \bm\mu\bm\mu^\top}  \\
            &= \frac{N-1}{N}\bm\Sigma \\
        \end{align*}
    \end{supframe}
    ...
    \begin{frame}{Derive a more general formulation of sequential learning}
        \begin{itemize}
            ...
            \item The regression function is given by:
                \begin{align*}
                    f(\theta) \equiv \mathbb E[z|\theta] = \int zp(z | \theta) \:\mathrm dz \tag{2.127}
                \end{align*}
                \begin{figure}[h]\centering
    ...
    \begin{frame}{Solve a general ML problem sequentially using the Robbins-Monro Algorithm}
        \begin{align*}
            z &= -\frac{\partial}{\partial\mu_\mathrm{ML}}\ln p(x | \mu_\mathrm{ML}, \sigma^2) = -\frac{1}{\sigma^2}(x - \mu_\mathrm{ML}) \tag{2.136} \\
            \mathbb E[z | \theta] &= \frac{1}{\sigma^2}(\mu - \mu_\mathrm{ML})
        \end{align*}
        \begin{figure}[h]\centering
    ...
    \begin{frame}{Recap: Bayesian inference for the binomial dist and conjugacy}
        The binomial distribution:
        \begin{align*}
            \mathrm{Bin}(m | N, \mu) = \begin{pmatrix} N \\ m \end{pmatrix} \mu^m (1 - \mu)^{N-m} \tag{2.9}
        \end{align*}

        In order to develop a Bayesian treatment to model the distribution of observations, 
        we need to introduce a prior $p(\mu)$.
        \begin{align*}
            p(\mu | N, m) \propto \mathrm{Bin}(m | N, \mu) p(\mu)
        \end{align*}

        If we choose a prior to be $c\mu^\beta(1-\mu)^\gamma$, 
        the corresponding posterior $p(\mu | N, m)$ will have the same functional form as the prior (\redtext{conjugacy}).
        
        The beta distribution satisfies the conjugacy
        \begin{align*}
    ...
    \begin{supframe}{Solution of Exercise 2.38 (1/2)}
        From the product rule of probability, 
        \begin{align*}
            \ln p(\mu | \mathbf X)
            &= \ln p(\mathbf X | \mu) + \ln p(\mu) + \mathrm{const.} \\
            &= \sum_{n=1}^N \ln p(x_n | \mu) + \ln p(\mu) + \mathrm{const.} \\
            &= -\frac{1}{2\sigma^2}\sum_{n=1}^N(x_n - \mu)^2 - \frac{1}{2\sigma_0^2}(\mu - \mu_0)^2 + \mathrm{const.} \\
            &= -\frac{1}{2\sigma^2}\sum_{n=1}^N(\mu^2 - 2\mu x_n + x_n^2) - \frac{1}{2\sigma_0^2}(\mu^2 - 2\mu\mu_0 + \mu_0^2) + \mathrm{const.} \\
            &= -\frac{1}{2\sigma^2}(N\mu^2 - 2\mu \abracket{x} + \abracket{x^2}) - \frac{1}{2\sigma_0^2}(\mu^2 - 2\mu\mu_0 + \mu_0^2) + \mathrm{const.} \\
            & \quad \nbracket{\abracket{x} \equiv \sum_{n=1}^N x_n, \; \abracket{x^2} \equiv \sum_{n=1}^N x_n^2} \\
        \end{align*}
    \end{supframe}
    
    \begin{supframe}{Solution of Exercise 2.38 (2/2)}
        \begin{align*}
            \ln p(\mu | \mathbf X)
            &= -\frac{1}{2\sigma^2}(N\mu^2 - 2\mu \abracket{x} + \abracket{x^2}) - \frac{1}{2\sigma_0^2}(\mu^2 - 2\mu\mu_0 + \mu_0^2) + \mathrm{const.} \\
            &= -\frac{1}{2\sigma^2}\cbracket{\nbracket{\frac{N}{\sigma^2} + \frac{1}{\sigma_0^2}}\mu^2 - 2\nbracket{\frac{\abracket{x}}{\sigma^2} + \frac{\mu_0}{\sigma_0^2}}\mu} + \mathrm{const.} \\
            &= -\frac{1}{2}\cdot\frac{N\sigma_0^2 + \sigma^2}{\sigma^2\sigma_0^2}
                \nbracket{\mu^2 - 2 \frac{\abracket{x}\sigma_0^2 + \mu_0\sigma^2}{N\sigma_0^2 + \sigma^2} \mu} + \mathrm{const.} \\
            &= -\frac{(\mu - \mu_N)^2}{2\sigma_N^2} + \mathrm{const.}
        \end{align*}
        where, 
        \begin{align*}
            \mu_N &= \frac{\sigma^2}{N\sigma_0^2 + \sigma^2}\mu_0 + \frac{N\sigma_0^2}{N\sigma_0^2 + \sigma^2}\mu_\mathrm{ML} \tag{2.141} \\
    ...
    \begin{frame}{Interpretation of the posterior's params}
        \begin{columns}
            \begin{column}{0.50\hsize}
                \begin{align*}
                    p(\mu | \mathbf X) &= \mathcal N(\mu | \mu_N, \sigma_N^2) \tag{2.140} \\
                    \mu_N &= \highlightcap<2->[red]{\displaystyle\frac{\sigma^2}{N\sigma_0^2 + \sigma^2}\mu_0 + \frac{N\sigma_0^2}{N\sigma_0^2 + \sigma^2}\mu_\mathrm{ML}}{\small $\mu_N$ intervene between $\mu_0$ and $\mu_\mathrm{ML}$} \tag{2.141} \\
    ...
    \begin{frame}{Demonstration of a Bayesian sequential estimation}
        See Jupyter Notebook.
    ...
    \begin{frame}{The task of inferring the precision $\lambda \equiv 1/\sigma^2$ (the mean $\mu$ is known)}
        \begin{itemize}
            ...
            \item Interpretation of the posterior's params
                \begin{itemize}
                    \item $a_N$ and $b_N$ can be interpret like an \textit{effective number of observations} (see \S2.2).
                    \item From (2.150) and (2.151), 
                        we see that introducing a prior $\mathrm{Gam}(\lambda | a_0, b_0)$ corresponds to
                        we have the $2a_0$ effective observations having variance $b_0/a_0$ in the prior.
                \end{itemize}
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{The conjugate priors in the case of the univariate Gaussian}
        \begin{itemize}
            \item For known mean $\mu$ and unknown precision $\lambda$, the conjugate prior is a Gaussian:
                \begin{align*}
                    p(\mu) = \mathcal N(\mu | \mu_0, \lambda_0^{-1})
                \end{align*}
            \item For unknown mean $\mu$ and known precision $\lambda$, the conjugate prior is the \textit{gamma} distribution:
                \begin{align*}
                    p(\lambda) = \mathrm{Gam}(\lambda | a_0, b_0)
                \end{align*}
            \item For both the mean and the precision are unknown, the conjugate prior is the \textit{normal-gamma} distribution:
                \begin{align*}
                    p(\mu, \lambda)
                    = \mathcal N(\mu | \mu_0, (\beta\lambda)^{-1}) \mathrm{Gam}(\lambda | a, b) \tag{2.154}
                \end{align*}
        \end{itemize}
    \end{frame}

    \begin{frame}{The conjugate priors in the case of the multivariate Gaussian}
        \begin{itemize}
